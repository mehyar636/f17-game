...
const CONFIG = {
  ...
  difficulty: {
    ...
    hard:   { keeperReact: 0.18, keeperRead: 0.75, defSpeed: 2.00, energyDrain: 0.85, label: 'LEGEND',  shotPowerBoost: 0.95, defenderTackleRange: 1.30, defenderAggression: 1.30, defenderCountBonus: 2,  tackleCooldown: 0.7 },
  },
...
const Mechanics = {
  ...
  updateEnemies(G, dt) {
    ...
    G.enemies.forEach((e) => {
      ...
      const aggression = diff.defenderAggression || 1.0;  // higher = snappier chase, less wobble
      ...
      if (d < chaseRange) {
        ...
        // Move toward target — smoothed (aggression speeds up direction-change response)
      ...
      if (G.ball.owner === 'player' && dist(p, e) < tackleRange && p.dashTime <= 0 && (e.tackleCool || 0) <= 0) {
        ...
        e.tackleCool = diffCfg.tackleCooldown || 1.0;  // shorter cooldown = harder difficulty
      }
  ...
  decideKeeperDive(G, targetY) {
    ...
    const readChance = clamp(readBase + goalsBonus + levelBonus, 0.10, 0.85);
  ...
  keeperSaves(G, info) {
    ...
    let saveMult;
    if (G.mode === 'penalty') {
      saveMult = G.diff === 'easy' ? 0.35 : G.diff === 'medium' ? 0.62 : 0.85;
    } else {
      saveMult = G.diff === 'easy' ? 0.65 : G.diff === 'medium' ? 0.88 : 1.0;
    }
  ...
  updateFullback(G, dt) {
    ...
    // (Easy: clamps at 880 = easier to get past; Hard: clamps at 1000 = harder)
    const maxFullbackX = G.diff === 'easy' ? 880 : G.diff === 'medium' ? 940 : 1000;
  ...
  return {
    ...
    setup() {
      ...
      if (mode === 'penalty') {
        ...
        // Arena: spread defenders — count scales with difficulty
