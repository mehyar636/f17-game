...
const Mechanics = {
    ...
    G.enemies.forEach((e, idx) => {
      ...
      if (ball.owner === 'none' && ballSpeed > 150) {
        ...
        if (isPresser) {
          ...
          // Others hold ZONE — shift home position toward ball's Y but stay in their X lane.
          // This creates a defensive shape that compresses, not collapses.
          // Shift X toward player only if ball is in their depth band (within 200px X of home)
          const ballNearby = Math.abs(p.x - e.homeX) < 220;
          const targetY = lerp(e.homeY, p.y, ballNearby ? 0.55 : 0.30);
          const targetX = ballNearby ? lerp(e.homeX, p.x + 30, 0.35) : e.homeX;
      ...
      let sepX = 0, sepY = 0;
      const SEP_RADIUS = 70;  // defenders try to stay ~70px apart
      ...
      if (!G.restarting && G.ball.owner === 'player' && dist(p, e) < tackleRange && p.dashTime <= 0 && (e.tackleCool || 0) <= 0) {
  ...
  updateFullback(G, dt) {
    ...
    const fbSpeedMult = Math.min(diff.defSpeed, 1.40);
    const speed = 230 * fbSpeedMult * (1 + (G.levelIdx || 0) * 0.08);
    if (d > 12) {
    ...
    // Lowered Legendary clamp so Faisal has space at the byline
    const maxFullbackX = G.diff === 'easy' ? 860 : G.diff === 'medium' ? 920 : 960;
    ...
    if (!G.restarting &&
        Math.hypot(p.x - fullback.x, p.y - fullback.y) < p.r + fullback.r + 10 &&
  ...
  updateSkillsCourseDefenders(G, dt) {
    ...
    G.enemies.forEach((e, idx) => {
      ...
      if (isPresser && G.ball.owner === 'player' && Math.hypot(p.x - e.x, p.y - e.y) < p.r + e.r &&
          p.dashTime <= 0 && e.tackleCool <= 0 && !G.restarting) {
        e.tackleCool = 1.5;
  ...
  return {
    ...
    setup() {
      ...
      if (mode === 'penalty') {
        ...
        // Arena: spread defenders across pitch zones (back-line + midfield + flanks)
        // count scales with difficulty
        const baseDefs = G.cfg.defenders || 2;
        const diffCfg = CONFIG.difficulty[G.diff];
        const defs = Math.max(1, baseDefs + (diffCfg.defenderCountBonus || 0));

        // Zone slots — ordered so 1st defender = central CB, then RB/LB, then mid, then wide mid
        // Spread across X (520..1020) and full Y (160..600) so they don't all stack near center
        const ph = CONFIG.pitch.h;
        const zoneSlots = [
          { x: 980, y: ph / 2,        zone: 'CB'  },  // central back
          { x: 920, y: 200,           zone: 'RB'  },  // right back
          { x: 920, y: ph - 200,      zone: 'LB'  },  // left back
          { x: 720, y: ph / 2,        zone: 'CM'  },  // central mid
          { x: 640, y: 240,           zone: 'RM'  },  // right mid
          { x: 640, y: ph - 240,      zone: 'LM'  },  // left mid
        ];
    ...
    drawModeOverlay(t) {
      ...
      if (mode === 'crossing') {
        ...
        if (G.wingPhase === 'byline' && G.crossZone) {
          ...
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          ctx.fillText(ready ? '✅ CROSS or SHOOT!' : '🏃 RUN TO BYLINE', (z.x1 + z.x2) / 2, z.y1 + 21);
        }
      }
    ...
    drawTeammate(t) {
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(ready ? '⚽ CROSS NOW' : 'WAITING...', tm.x, tm.y - 51);
