    ...
    G.enemies.forEach((e, idx) => {
      ...
      if (ball.owner === 'none' && ballSpeed > 150) {
        ...
        if (isPresser) {
          ...
          // X shift: 30% if same side, less if opposite side
          const xShift = dxToBall < 250 ? 0.40 : 0.20;
          // Y shift: 50% if ball nearby in Y, less if far
          const yShift = dyToBall < 200 ? 0.55 : 0.30;
          // Compression caps so defenders don't run all the way to the player
          const maxXShift = 90;   // never shift more than 90px from home X
          const maxYShift = 140;  // never shift more than 140px from home Y
