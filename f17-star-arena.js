...
const CONFIG = {
  ...
  // NB: defSpeed ratio is 1.00 / 1.30 / 1.60 — smoother curve than v1 (was 1.10 / 1.45 / 1.75)
  // saveFloor lets each tier have a distinct minimum direct-hit save chance
  difficulty: {
    easy:   { keeperReact: 0.60, keeperRead: 0.18, defSpeed: 1.00, energyDrain: 0.20, label: 'ROOKIE',  shotPowerBoost: 1.30, defenderTackleRange: 0.80, saveFloor: 0.18, penaltyReactMult: 0.55 },
    medium: { keeperReact: 0.32, keeperRead: 0.45, defSpeed: 1.30, energyDrain: 0.55, label: 'PRO',     shotPowerBoost: 1.05, defenderTackleRange: 1.00, saveFloor: 0.30, penaltyReactMult: 0.65 },
    hard:   { keeperReact: 0.18, keeperRead: 0.65, defSpeed: 1.60, energyDrain: 0.95, label: 'LEGEND',  shotPowerBoost: 0.95, defenderTackleRange: 1.20, saveFloor: 0.42, penaltyReactMult: 0.75 },
  },
  ...
  energyHit: {
    ...
    bossBlock: 18,
    defenderBump: 6,
    missedShot: 4,        // missed the goal entirely
    savedShot: 3,         // keeper saved it
    dashUse: 2,           // each dash costs energy
    gateMissed: 5,        // wrong gate / order in skills course
  },
...
function refreshHome() {
  ...
  // Mode stars + progress
  const completed = Save.data.completedLevels || {};
  ['arena', 'penalty', 'crossing', 'boss'].forEach((mode) => {
    ...
    const progEl = document.querySelector(`[data-mode-progress="${mode}"]`);
    if (progEl) {
      const total = CONFIG.levels[mode].length;
      let done = 0;
      for (let i = 0; i < total; i++) if (completed[mode + ':' + i]) done++;
      progEl.textContent = `${done}/${total}`;
      progEl.classList.toggle('complete', done === total);
    }
  });
  ...
  // Difficulty hint
  const hintEl = $('diffHint');
  if (hintEl) {
    const hints = {
      easy:   '— forgiving keeper, slower defenders, full power',
      medium: '— balanced — closer to real football',
      hard:   '— sharp keeper, fast defenders, less margin',
    };
    hintEl.textContent = hints[Save.data.difficulty] || '';
  }
  ...
  row.innerHTML = Object.entries(ACHIEVEMENTS).map(([id, a]) => {
    ...
    return `<button class="ach-pill ${u ? 'unlocked' : ''}" data-ach-id="${id}" title="${a.name}: ${a.desc}">${a.icon}</button>`;
  }).join('');
  // Tap to show achievement details on mobile
  document.querySelectorAll('.ach-pill').forEach((p) => {
    p.onclick = (e) => {
      e.stopPropagation();
      const id = p.dataset.achId;
      const a = ACHIEVEMENTS[id];
      if (!a) return;
      const u = !!Save.data.achievements?.[id];
      Audio.buttonTap();
      showModal(`
        <div class="modal-icon">${u ? a.icon : '🔒'}</div>
        <h2>${a.name}</h2>
        <p>${a.desc}</p>
        <p style="opacity:0.6;font-size:11px;margin-top:8px;">${u ? '✓ UNLOCKED' : 'Locked — keep playing!'}</p>
...
$('resetBtn').addEventListener('click', () => {
  ...
  showModal(`
    ...
    <p>All stars, completed levels and achievements will be erased.</p>
    <p style="opacity:0.6;font-size:11px;">This cannot be undone.</p>
    <button class="btn-primary text-btn-danger-bg" id="mConfirm">RESET EVERYTHING</button>
...
function showModal(html, opts = {}) {
  ...
  modal.hidden = false;
  modal._dismissable = opts.dismissable !== false;  // default dismissable
}
function hideModal() {
  modal.hidden = true;
  // Animate fade-out by clearing the panel after transition
}
// Backdrop-tap dismiss for help/intro modals (not result modals which have buttons)
modal.addEventListener('click', (e) => {
  if (e.target === modal && modal._dismissable) {
    hideModal();
  }
});
...
function showModeIntro(mode) {
  ...
  // Build level picker — unlocked levels = L1 always + any after a completed prior level
  const completed = Save.data.completedLevels || {};
  const levelButtons = cfg.map((lvl, i) => {
    const isUnlocked = i === 0 || completed[mode + ':' + (i - 1)];
    const lvlStars = completed[mode + ':' + i] || 0;
    const starHtml = [0,1,2].map(s => `<span class="lvl-star ${s < lvlStars ? 'on' : ''}">★</span>`).join('');
    const lockIcon = isUnlocked ? '' : '🔒 ';
    return `<button class="lvl-btn ${isUnlocked ? '' : 'locked'}" data-level="${i}" ${isUnlocked ? '' : 'disabled'}>
      <div class="lvl-name">${lockIcon}L${i+1} • ${lvl.name}</div>
  ...
  showModal(`
    ...
    <div class="lvl-picker-label">SELECT LEVEL</div>
    <div class="lvl-picker">${levelButtons}</div>
    <button class="btn-secondary" id="mCancel">BACK</button>
  `);
  // Wire level buttons — each starts at the chosen level
  document.querySelectorAll('.lvl-btn').forEach((btn) => {
    if (btn.disabled) return;
    btn.onclick = () => {
      Audio.buttonTap();
      hideModal();
      startLevel(mode, parseInt(btn.dataset.level));
    };
  });
  $('mCancel').onclick = () => { Audio.buttonTap(); hideModal(); };
}
...
const HUD = {
  ...
  setCoach(text, duration = 4000) {
    ...
    // Don't show coach if a toast is currently visible — avoid two competing messages
    const toastEl = $('toast');
    if (toastEl && !toastEl.hidden) {
      // Defer the coach until after the toast clears
      clearTimeout(this._coachT);
      this._coachT = setTimeout(() => HUD.setCoach(text, duration), 700);
      return;
    }
  ...
  setScoreboard(home, away, awayLabel, mode, level) {
    ...
    // Home label changes per mode for clarity
    const homeLabelEl = $('sbHomeLabel');
    if (homeLabelEl) {
      const homeLabels = {
        'STAR ARENA':  '⚽ GOALS',
        'FREE KICK':   '⚽ GOALS',
      ...
      homeLabelEl.textContent = homeLabels[mode] || '⚽ GOALS';
    }
  },
  ...
  // Cooldown indicators on action buttons
  setDashCooldown(remainingSec, totalSec) {
    const btn = $('dashBtn');
    if (!btn) return;
    if (remainingSec > 0.05 && totalSec > 0) {
      const pct = (remainingSec / totalSec) * 360;
      btn.style.setProperty('--cd-pct', pct + 'deg');
      btn.classList.add('cooldown');
    } else {
      btn.classList.remove('cooldown');
    }
  },
  // Wing Attack: only enable CROSS button when at byline
...
const Mechanics = {
  ...
  updateBall(G, dt) {
    ...
    // Cool down lastTouch — guard against spawning a timer every frame
    if (b.lastTouch === 'just-shot' && !b._coolingDown) {
      b._coolingDown = true;
      setTimeout(() => {
        if (G.ball.lastTouch === 'just-shot') G.ball.lastTouch = 'shot';
        G.ball._coolingDown = false;
      }, 350);
    }
  },
  ...
  shoot(G) {
    ...
    let targetX, targetY;
    if (G.mode === 'crossing') {
      // SHOOT button in Wing Attack = direct goal shot (uses joystick aim like Arena)
      // CROSS button (PASS) routes to teammate via doWingCross()
      // This branch is reached only via SHOOT, so always aim at goal
  ...
  decideKeeperDive(G, targetY) {
    const k = G.keeper;
    const diff = CONFIG.difficulty[G.diff];
    // Penalty react multiplier per-difficulty (was 0.35 hardcoded - made keeper FASTER on penalty than open play, inverted intent)
    // Now: easy 0.55, medium 0.65, hard 0.75 — keeper still slower than open play but scales with diff
    let baseReact = G.mode === 'penalty' ? diff.keeperReact * diff.penaltyReactMult : diff.keeperReact;
  ...
  keeperSaves(G, info) {
    ...
    // Per-difficulty save floor — was 0.40 hardcoded for all tiers (made all difficulties identical on hard shots)
    const floor = diff.saveFloor || 0.30;
    ...
    if (dy < reach) {
      ...
      sc *= saveMult;
      sc = clamp(sc, floor, 0.95);  // floor scales with difficulty (0.18 / 0.30 / 0.42)
  ...
  handleSave(G) {
    ...
    G.keeper.anim = 'dive';
    G.energy = Math.max(0, G.energy - CONFIG.energyHit.savedShot * CONFIG.difficulty[G.diff].energyDrain);
    Audio.save();
    hapticPattern([100, 60, 30]);  // longer first pulse — feels like keeper "catch"
  ...
  handleMiss(G) {
    G.combo = 0;
    G.energy = Math.max(0, G.energy - CONFIG.energyHit.missedShot * CONFIG.difficulty[G.diff].energyDrain);
    Audio.miss();
    hapticPattern([40]);
  ...
  setupWingAttack(G) {
    ...
    // Delay the start-toast slightly so it doesn't fight with the countdown overlay
    const sideToast = G.wingSide === 'right' ? 'RIGHT WING ATTACK!' : 'LEFT WING ATTACK!';
    setTimeout(() => {
      if (G.running && G.countdown <= 0) HUD.showToast(sideToast, 'gold', 1100);
    }, G.countdown > 0 ? G.countdown * 1000 + 100 : 50);
  },
  ...
  updateWingAttack(G, dt) {
    ...
    if (G.wingPhase === 'beat') {
      ...
      //   (c) is very far past the defender (>180px) — sneaky walk-around
      // Threshold lowered from 60 to 40 so dash bursts actually create a "beat" event
      const past = p.x > fullback.x + 40;
      const dashing = p.dashTime > 0;
      const fast = Math.hypot(p.vx, p.vy) > 260;
      const veryFarPast = p.x > fullback.x + 180;
  ...
  updateFullback(G, dt) {
    ...
    // Block the player's path on the wing — TRACK player tightly (not lead)
    // Leading the player makes dash escapes impossible because defender cuts the angle
    const targetX = p.x;        // pure track, no lead
    const targetY = p.y;
    const dx = targetX - fullback.x;
    const dy = targetY - fullback.y;
    const d = Math.hypot(dx, dy);
    const diff = CONFIG.difficulty[G.diff];
    // Base speed lowered from 250 → 210, level scaling kept at +12%/level
    // Result on ROOKIE L1: 210 px/s (vs player walk 240 / dash 480) — beatable
    // Result on LEGEND L3: 210 × 1.60 × 1.24 = 416 px/s — challenging but still under dash
    const speed = 210 * diff.defSpeed * (1 + (G.levelIdx || 0) * 0.12);
    if (d > 12) {
    ...
    // Apply difficulty tackleRange multiplier (was hardcoded +10 — bigger than any arena defender)
    // Also add post-dash grace: player can't be tackled for 0.25s after dash ends
    const postDashGrace = (fullback.dashGraceTime || 0) > 0;
    if (p.dashTime > 0) fullback.dashGraceTime = 0.25;
    fullback.dashGraceTime = Math.max(0, (fullback.dashGraceTime || 0) - dt);

    const tackleRange = (p.r + fullback.r) * diff.defenderTackleRange;
    if (Math.hypot(p.x - fullback.x, p.y - fullback.y) < tackleRange &&
        p.dashTime <= 0 && !postDashGrace && (fullback.tackleCool || 0) <= 0) {
      fullback.tackleCool = 1.5;
      Mechanics.tackle(G, fullback);
      // Lighter push back — was 110, now 70
      p.x -= 70;
      p.vx = -30;
      HUD.setCoach('Blocked! Use DASH (yellow) earlier to beat the fullback.');
      // CRITICAL FIX: tackle in Wing Attack must advance the attack counter
      // Otherwise player loops forever on the same attack with no progress
      setTimeout(() => { if (G.running) Mechanics.startNextWingAttack(G); }, 1100);
    }
  ...
  updateSkillsCourseDefenders(G, dt) {
    ...
    G.enemies.forEach(e => {
      ...
      if (G.ball.owner === 'player' && Math.hypot(p.x - e.x, p.y - e.y) < p.r + e.r &&
        ...
        e.tackleCool = 1.5;
        // Energy drain so repeated tackles eventually fail the level
        G.energy = Math.max(0, G.energy - CONFIG.energyHit.bossBlock * CONFIG.difficulty[G.diff].energyDrain);
  ...
  doDash(G) {
    ...
    G.energy = Math.max(0, G.energy - CONFIG.energyHit.dashUse * CONFIG.difficulty[G.diff].energyDrain);
    Audio.dash();
    hapticPattern([15]);
...
function drawAimGuide(G) {
  ...
  const b = G.ball;
  const charge = G.charge;
  // SHOOT charge in Wing Attack now aims at goal (matches new shoot logic)
  // CROSS button does its own routing — no charge guide needed for crosses
  const target = { x: CONFIG.pitch.w - 50, y: CONFIG.pitch.h/2 + charge.aimY * (CONFIG.goal.h * 0.45) };
...
function makeGameScene(mode) {
  return {
    ...
    refreshHUD() {
      ...
      // Cooldown indicators
      HUD.setDashCooldown(G.player.dashCool, CONFIG.player.dashCooldown);
      // Cross button only enabled at byline in crossing mode
      if (mode === 'crossing') {
        HUD.setCrossEnabled(G.wingByline && !G.crossDelivered);
      }
    },
    ...
    drawTeammate(t) {
      ...
      ctx.stroke();
      ctx.setLineDash([]);
      // Label — bigger, fitted to text width so nothing clips
      const labelText = ready ? '⚽ CROSS NOW' : 'PASS HERE';
      ctx.font = 'bold 18px sans-serif';
      const tw = ctx.measureText(labelText).width;
      const padX = 14, padY = 6, labelH = 32;
      const labelW = Math.ceil(tw + padX * 2);
      ctx.fillStyle = color;
      roundRect(ctx, tm.x - labelW / 2, tm.y - 70, labelW, labelH, 16);
      ctx.fillStyle = '#0c1a14';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(labelText, tm.x, tm.y - 70 + labelH / 2);
...
setTimeout(() => { if (assetsLoaded < 2) boot(); }, 1200);
